...
# --- 2. محرك حساب الأيام خلف الكواليس ---
def extract_report_days(df):
    date_strs = set()
...
def create_modern_excel(df, client_name):
    ...
    if 'ربح الشركة الصافي' in df.columns:
        ...
        ws_data.conditional_format(f'{col_let}4:{col_let}{max_data_row}', {'type': 'cell', 'criteria': '<', 'value': 0, 'format': red_fmt})
    ...
    if len(df) > 1 and 'اسم المندوب' in df.columns:
        ...
        chart.add_series({'name': 'الطلبات المحققة', 'categories': f'=البيانات التفصيلية!${name_idx}$4:${name_idx}${max_data_row}', 'values': f'=البيانات التفصيلية!${order_idx}$4:${order_idx}${max_data_row}'})
...
if perf_file and car_fuel_file:
    with st.spinner('⏳ جاري المسح وحساب الإجماليات...'):
        ...
        report_days = extract_report_days(df_perf)
